...
The owner can freeze the release branch (for example while an assessor reviews a submission). While `.whole-team.json` has `"freeze": {"release": true}`, do not open pull requests into it, tag, deploy to production or hotfix. Work continues on the integration branch. Until the owner says to lift the freeze, answer any request that would touch the release branch by pointing to the freeze, and do not carry it out.
...
3. Log a failure as a bug (`fix/` branch, failing test first). Never mark a failed scenario passed.
...
5. Send scenarios that need human judgement (copy, look and feel, legal wording) to the owner with the evidence attached.
...
A short honest review before the first production release: security controls in place, monitoring and alerting, backups, error handling, performance under expected load, known limitations. Write down anything missing with its risk; do not hide gaps.
...
Under "What I would do next", list the deferred items with what each would need, in priority order. Assessors read it as judgement.
