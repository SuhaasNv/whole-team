...
Use the whole-team skill. Show this cheat sheet, then run `wt.py status` if the project has a .whole-team.json and suggest the single most useful next command.
...
| /whole-team:sprint | Plan the next sprint: goal, ready stories, capacity, owner's yes |
| /whole-team:status | Daily start: where the sprint stands and what is next |
| /whole-team:story | Build the next story (or a named one) end to end |
| /whole-team:bug | Something is broken |
| /whole-team:code-review | Independent review of the current branch |
| /whole-team:handover | You are about to stop for the day |
| /whole-team:retro | Retrospective on its own |
